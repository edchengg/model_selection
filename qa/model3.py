...
class CoNLLProcessor(object):
    ...
    def read_pkl(self, target_task, target_language, feature1, feature2):
        ...
        self.lang_dic = {'ar':0, 'de':1, 'es':2, 'zh':3, 'hi':4, 'vi':5}
        # create training
        # create dev
        lang_list = ['ar', 'de', 'es', 'zh']
        ...
        dev_target_examples = self.create_eval_examples(data, dev_splits1, feature1.replace('**', target_language),
                                                      'dev_**_f1'.replace('**', target_language),
                                                      task='qa', lang=target_language)
        # create test
        test_examples = self.create_eval_examples(data, test_splits1, feature1.replace('**', target_language),
                                                  'test_**_f1'.replace('**', target_language),
                                                  task='qa', lang=target_language)

        self.run_dev_baseline(data, dev_splits1, 'dev_**_f1'.replace('**', target_language))
        self.run_test_baseline(data, test_splits1, 'test_**_f1'.replace('**', target_language))

       #t('Train data size: ', len(train_examples))
