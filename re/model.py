...
class CoNLLProcessor(object):
    ...
    def read_pkl(self, target_task, target_language, feature1, feature2):
        ...
        if target_task == 're':
            ...
            dev_target_examples = self.create_eval_examples(data, dev_splits1, feature1.replace('**', target_language),
                                                          're_**_dev_f1'.replace('**', target_language),
                                                          task='re', lang=target_language)
            # create test
            test_examples = self.create_eval_examples(data, test_splits1, feature1.replace('**', target_language),
                                                      're_**_test_f1'.replace('**', target_language),
                                                      task='re', lang=target_language)

            self.run_dev_baseline(data, dev_splits1, 're_**_dev_f1'.replace('**', target_language))
            self.run_test_baseline(data, test_splits1, 're_**_test_f1'.replace('**', target_language))
        else:
            self.lang_dic = {'ar': 0, 'zh': 1, 'en': 2}
            # create training
            # create dev
            ...
            for lang in lang_list:
                tmp_train_examples = self.create_train_examples(data, train_splits1, feature2.replace('**', lang),
                                                                'arl_**_dev_f1'.replace('**', lang), task='arl',
                                                                lang=lang)
                tmp_dev_examples = self.create_eval_examples(data, dev_splits1, feature2.replace('**', lang),
                                                             'arl_**_dev_f1'.replace('**', lang), task='arl', lang=lang)
                train_examples.extend(tmp_train_examples)
                dev_examples.append(tmp_dev_examples)

            # create test
            dev_target_examples = self.create_eval_examples(data, dev_splits1, feature2.replace('**', target_language),
                                                      'arl_**_dev_f1'.replace('**', target_language), task='arl',
                                                      lang=target_language)
            # create test
            test_examples = self.create_eval_examples(data, test_splits1, feature2.replace('**', target_language),
                                                      'arl_**_test_f1'.replace('**', target_language), task='arl',
                                                      lang=target_language)

            self.run_dev_baseline(data, dev_splits1, 'arl_**_dev_f1'.replace('**', target_language))
            self.run_test_baseline(data, test_splits1, 'arl_**_test_f1'.replace('**', target_language))
    ...
    def create_train_examples(self, data, splits, feature, label_f1, task, lang):
        if task == 're':
            feature_target = 're_' + self.target_language + '_dev_repr'
            feature_en = 're_en_dev_repr'
            feature_pivot = 're_' + lang + '_dev_repr'
            feature_paral = 're_en2' + lang + '_repr'
        elif task == 'arl':
            feature_target = 'arl_' + self.target_language + '_dev_repr'
            feature_en = 'arl_en_dev_repr'
            feature_pivot = 'arl_' + lang + '_dev_repr'
            feature_paral = 'arl_en2' + lang + '_repr'
