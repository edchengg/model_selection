...
MODEL_CLASSES = {
  "bert": (BertConfig, BertForTokenClassificationCLS, BertTokenizer),
...
def extract_cls(args, model, tokenizer, labels, pad_token_label_id, mode, prefix="", lang="en", lang2id=None, print_result=True):
  eval_dataset = load_and_cache_examples(args, tokenizer, labels, pad_token_label_id, mode=mode, lang=lang, lang2id=lang2id)
  ...
  for batch in tqdm(eval_dataloader, desc="Evaluating"):
    ...
    for i in range(len(cls_token)):
      y_feature.append(cls_token[i])

  avg_rep = np.mean(y_feature, axis=0, keepdims=True)[0]

  output_feature_file = os.path.join(args.output_dir, 'repr_{}.npz'.format(lang))

  np.save(output_feature_file, avg_rep)
...
def load_and_cache_examples(args, tokenizer, labels, pad_token_label_id, mode, lang, lang2id=None, few_shot=-1):
  ...
  if os.path.exists(cached_features_file) and not args.overwrite_cache:
    ...
    for lg in langs:
      if 'pseudo' in mode:
        if lg == 'en':
          data_file = os.path.join(args.data_dir, lg, "{}.{}".format('train', args.model_name_or_path))
        else:
...
def main():
  ...
  parser.add_argument("--data_dir",
                      default='/srv/scratch/ychen3411/project00_model_selection/dataset/panx_processed_maxlen128/',
                      type=str,
  ...
  parser.add_argument("--labels",
                      default="/srv/scratch/ychen3411/project00_model_selection/dataset/panx_processed_maxlen128/labels.txt",
                      type=str,
  ...
  parser.add_argument("--do_predict_train", action="store_true",
                      help="Whether to run predictions on the train set.")
  ...
  parser.add_argument("--log_file", type=str, default='xxxx', help="log file")
  ...
  parser.add_argument("--use_pseudo_labels", type=str, default="[NONE]",
                      help="Whether use pseudo labels in languages other than en")
  ...
  if args.mask_dir:
    ...
    print('Model zerorate: ', zero_rate)

  logger.info("Training/evaluation parameters %s", args)

  # Training
  if args.do_train:
    if args.use_pseudo_labels != '[NONE]':
      train_dataset = load_and_cache_examples(args, tokenizer, labels, pad_token_label_id, mode=args.use_pseudo_labels,
  ...
  #     model.to(args.device)
  #   else:
  #     # with mask, the model params name is different so we need to prune then load
  #     model = model_class.from_pretrained(args.model_name_or_path, config=config)
  #     model.to(args.device)
  #     pruning_model_custom(model, mask)
  #     zero_rate = see_weight_rate(model)
  #     print('Model zerorate: ', zero_rate)
  #     model.load_state_dict(torch.load(best_checkpoint + '/pytorch_model.bin', map_location=args.device))
  #     model.to(args.device)
  #
  #
    ...
    with open(output_test_results_file, "w") as result_writer:
      for lang in args.predict_langs.split(','):
        if not os.path.exists(os.path.join(args.data_dir, lang, 'dev.{}'.format(args.model_name_or_path))):
          logger.info("Language {} does not exist".format(lang))
          continue
        extract_cls(args, model, tokenizer, labels, pad_token_label_id, mode="dev", lang=lang, lang2id=lang2id)
  ...
  if args.do_predict_train and args.local_rank in [-1, 0]:
    ...
    output_train_results_file = os.path.join(args.output_dir, "train_results.txt")
    with open(output_train_results_file, "a") as result_writer:
      for lang in args.predict_langs.split(','):
        if not os.path.exists(os.path.join(args.data_dir, lang, 'train.{}'.format(args.model_name_or_path))):
          logger.info("Language {} does not exist".format(lang))
          continue
        result, predictions = evaluate(args, model, tokenizer, labels, pad_token_label_id, mode="train", lang=lang,
                                       lang2id=lang2id)
