...
for sh in range(10):
    tmp_file = open('ex_bash/run_{}.sh'.format(sh), 'w')

    tmp_file.write('#!/bin/sh -l\n')
    tmp_file.write('#SBATCH --job-name=ex-{}\n'.format(sh))
    tmp_file.write('#SBATCH --output=ex-{}-log\n'.format(sh))
    tmp_file.write('#SBATCH --error=ex-{}-err\n'.format(sh))
    ...
    for _ in range(24):
        ...
        tmp_file.write('python $REPO/run_tag_cls_rep.py \
...
--save_only_best_checkpoint'.format(output_dir, seed))
        tmp_file.write('\n\n')
        seed += 1
    tmp_file.close()