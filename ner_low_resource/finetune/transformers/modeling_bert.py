...
@add_start_docstrings(
    """Bert Model extract CLS token with a token classification head on top (a linear layer on top of
    the hidden-states output) e.g. for Named-Entity-Recognition (NER) tasks. """,
    BERT_START_DOCSTRING,
)
class BertForTokenClassificationCLS(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
    ...
    Examples::
        ...
        return outputs, cls_token  # (loss), scores, (hidden_states), (attentions)
