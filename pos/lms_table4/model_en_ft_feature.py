...
class CoNLLProcessor(object):
    ...
    def read_pkl(self, target_language):
        ...
        read file
        '''
        task_file = "pos18.pkl" # mBERT representation - finetuned on English POS dataset only
        ...
        for lang in lang_list:
            tmp_train_examples = self.create_train_examples(data, train_splits1, 'ud_**_dev_repr'.replace('**', lang),
                                                            'ud_dev_**_acc'.replace('**', lang),
                                                            task='pos', lang=lang)
            tmp_dev_examples = self.create_eval_examples(data, dev_splits1, 'ud_**_dev_repr'.replace('**', lang),
        ...
        dev_target_examples = self.create_eval_examples(data, dev_splits1, 'ud_**_dev_repr'.replace('**', target_language),
                                                        'ud_dev_**_acc'.replace('**', target_language),
                                                        task='pos', lang=target_language)
        # create test
        test_examples = self.create_eval_examples(data, test_splits1, 'ud_**_dev_repr'.replace('**', target_language),
                                                  'ud_test_**_acc'.replace('**', target_language),
                                                  task='pos', lang=target_language)
