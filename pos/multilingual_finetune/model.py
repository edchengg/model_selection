...
def create_dataloader_cross_lingual(train_examples, add_data=None, set_type='train', batchsize=32, max_seq_length=128, tokenizer=None, num_duplicate=20):
    proc = UDProcessor()

    data_examples = train_examples
    data_size = len(data_examples)
    print('Dataset set: %s, orig size: %s' %(set_type, data_size))
...
class BertForPOS(BertForTokenClassification):
    ...
    def extract_cls(self, input_ids, token_type_ids=None, attention_mask=None):
        output = self.bert(input_ids, token_type_ids=token_type_ids, attention_mask=attention_mask)
        sequence_output = output[0]
    ...
    def valid_word_output(self, sequence_output, valid_ids):
        ...
        valid_output = torch.zeros(batch_size, max_len, feat_dim, dtype=torch.float32, device=self.gpu)
...
def evaluate_save(model, eval_dataloader, data_size):
    model.eval()
    device = model.get_device()
    label_map = model.get_label_map()
    y_true = [[] for _ in range(data_size)]
    ...
    acc = sum(1 for x,y in zip(flat_y_true, flat_y_pred) if x == y) / len(flat_y_true)
    avg_dev_loss = dev_loss / len(eval_dataloader)
    return acc, avg_dev_loss, flat_y_true, flat_y_pred
...
def extract_features(model, dataloader, data_size):
    model.eval()
    device = model.get_device()

    y_pred = [[] for _ in range(data_size)]

    for input_ids, input_mask, segment_ids, label_ids, valid_ids, l_mask, eval_idx, sen_id in dataloader:
        input_ids = input_ids.to(device)
        input_mask = input_mask.to(device)
        segment_ids = segment_ids.to(device)
        sen_id = sen_id.to('cpu').numpy()
        ...
        with torch.no_grad():
            cls_token = model.extract_cls(input_ids, segment_ids, input_mask)

        cls_token = cls_token.detach().cpu().numpy()

        for i, s_id in enumerate(sen_id):
            idx = s_id[0]
            y_pred[idx].append(cls_token[i])

    # average rep
    y_res = [[] for _ in range(data_size)]
    for i in range(data_size):
        if len(y_pred) != 1:
            x = np.mean(y_pred[i], axis=0, keepdims=True)[0]
        else:
            x = y_pred[i]
        y_res[i] = x

    avg_rep = np.mean(y_res, axis=0, keepdims=True)[0]
