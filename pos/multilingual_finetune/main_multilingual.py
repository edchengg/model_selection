...
if __name__ == '__main__':
    ...
    train_examples = []
    for lang in ['en', 'zh', 'de', 'es', 'nl', 'ar']:
        if lang == 'en':
            split = 'train'
        else:
            split = 'dev'
        tmp_path = 'UD_' + lang2full[lang] + '/' + lang + '-ud-{}.conllu'.format(split)
        train_examples += processor.get_examples(tmp_path)
    ...
    train_dataloader, _ = create_dataloader_cross_lingual(train_examples, set_type='train', batchsize=args.batchsize,
